# helix-editor/helix

[![Stars](https://img.shields.io/github/stars/helix-editor/helix?style=flat-square&color=yellow)](https://github.com/helix-editor/helix/stargazers) [![Forks](https://img.shields.io/github/forks/helix-editor/helix?style=flat-square&color=blue)](https://github.com/helix-editor/helix/network) [![Language](https://img.shields.io/badge/lang-Rust-informational?style=flat-square)](#) [![Score](https://img.shields.io/badge/score-95%2F100-brightgreen?style=flat-square)](#)

> A post-modern modal text editor.

## 📊 Stats

| | |
|---|---|
| ⭐ **Stars** | 44.2k |
| 🍴 **Forks** | 3.4k |
| 💻 **Language** | Rust |
| 📈 **Score** | 95/100 |
| 🗓️ **Last push** | — |
| 🔍 **Source** | github |

## 🏷️ Topics

_no topics_

## 🎯 Categories

Misc

## 📝 Summary

Helix is an open-source, post-modern modal text editor designed to provide a unique and efficient writing experience. It focuses on minimalism and simplicity while incorporating innovative features to enhance user productivity. This project aims to offer a lightweight and customizable alternative to traditional text editors.

---

<sub>🔭 Discovered 2026-04-24 · [View on GitHub](https://github.com/helix-editor/helix) · [← Back to Misc](./README.md)</sub>
