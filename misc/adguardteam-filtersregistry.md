# AdguardTeam/FiltersRegistry

[![Stars](https://img.shields.io/github/stars/AdguardTeam/FiltersRegistry?style=flat-square&color=yellow)](https://github.com/AdguardTeam/FiltersRegistry/stargazers) [![Forks](https://img.shields.io/github/forks/AdguardTeam/FiltersRegistry?style=flat-square&color=blue)](https://github.com/AdguardTeam/FiltersRegistry/network) [![Language](https://img.shields.io/badge/lang-Adblock%20Filter%20List-informational?style=flat-square)](#) [![Score](https://img.shields.io/badge/score-43%2F100-brightgreen?style=flat-square)](#)

> Known filters subscriptions transformed for better compatibility with AdGuard

## 📊 Stats

| | |
|---|---|
| ⭐ **Stars** | 341 |
| 🍴 **Forks** | 71 |
| 💻 **Language** | Adblock Filter List |
| 📈 **Score** | 43/100 |
| 🗓️ **Last push** | 2026-05-09 |
| 🔍 **Source** | github |

## 🏷️ Topics

`adguard` `filters` `filters-metadata` `filters-subscriptions`

## 🎯 Categories

Misc

## 📝 Summary

### English

AdguardTeam/FiltersRegistry is an open‑source collection that converts popular filter subscriptions into formats optimized for use with AdGuard products, ensuring smoother integration and improved blocking performance. By standardizing and enhancing compatibility, the project helps users maintain robust ad‑blocking, privacy, and security rules across AdGuard’s ecosystem. It is categorized under Payments and Data, reflecting its role in managing subscription data and related licensing information.

### Русский

AdguardTeam/FiltersRegistry — это открытый репозиторий, в котором собраны и преобразованы известные подписки фильтров (рекламных, трекеров, родительского контроля и пр.) для обеспечения их полной совместимости с продуктами AdGuard. Проект упрощает интеграцию и обновление фильтров, предоставляя готовый к использованию набор списков, а также поддерживает прозрачность и контроль над данными, связанными с блокировкой. Он относится к категориям Payments и Data, поскольку затрагивает вопросы лицензирования фильтров и обработки пользовательских данных.

### 中文

AdguardTeam/FiltersRegistry：AdguardTeam/FiltersRegistry may be useful when its README and activity match a concrete workflow.。适合用于Misc。Early or unclear: treat as research material until maintenance, releases, docs, and issue activity are verified.

## 🧭 Practical evaluation

**Value:** AdguardTeam/FiltersRegistry may be useful when its README and activity match a concrete workflow.

**Best use cases**

- Not enough metadata yet.

**Integration notes:** Looks feasible to evaluate, but integration should start with a small proof of concept and README check.

**Production readiness:** Early or unclear: treat as research material until maintenance, releases, docs, and issue activity are verified.

**Quality signals**

- 341 GitHub stars
- 71 forks
- updated 2026-05-09
- primary language: Adblock Filter List
- 4 topics

**Risks:** Integration path is not obvious from metadata; validate setup cost before committing.

## 🧮 Score breakdown

| Dimension | Score |
|---|---:|
| forks | 46/100 |
| stars | 54/100 |
| topics | 50/100 |
| outlook | 43/100 |
| quality | 51/100 |
| recency | 20/100 |
| adoption | 52/100 |
| production | 44/100 |
| usefulness | 42/100 |
| integration | 30/100 |
| sourceTrust | 100/100 |

---

<sub>🔭 Discovered 2026-05-09 · [View on GitHub](https://github.com/AdguardTeam/FiltersRegistry) · [← Back to Misc](./README.md)</sub>
