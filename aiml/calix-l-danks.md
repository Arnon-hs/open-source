# Calix-L/DanKS

[![Stars](https://img.shields.io/github/stars/Calix-L/DanKS?style=flat-square&color=yellow)](https://github.com/Calix-L/DanKS/stargazers) [![Forks](https://img.shields.io/github/forks/Calix-L/DanKS?style=flat-square&color=blue)](https://github.com/Calix-L/DanKS/network) [![Language](https://img.shields.io/badge/lang-Python-informational?style=flat-square)](#) [![Score](https://img.shields.io/badge/score-54%2F100-brightgreen?style=flat-square)](#)

> RL‑Empowered Small‑Scale Competitive Guandan Agent

## 📊 Stats

| | |
|---|---|
| ⭐ **Stars** | 110 |
| 🍴 **Forks** | 2 |
| 💻 **Language** | Python |
| 📈 **Score** | 54/100 |
| 🗓️ **Last push** | 2026-09-05 |
| 🔍 **Source** | story-link |

## 🏷️ Topics

`card-games` `game-ai` `guandan` `pytorch` `reinforcement-learning`

## 🎯 Categories

AI/ML

## 📝 Summary

### English

Calix-L/DanKS: . Use it for AI/ML. 

### Русский

Calix-L/DanKS: open-source проект в категориях AI/ML. Практическое применение: нужна ручная оценка сценария. Уровень готовности: подходит для прототипа или внутреннего workflow, перед production нужна ручная проверка.

### 中文

Calix-L/DanKS：。适合用于AI/ML。

## 🧭 Practical evaluation

**Value:** Manual review required.

**Best use cases**

- Not enough metadata yet.

**Integration notes:** No integration notes yet.

**Production readiness:** Production readiness has not been assessed yet.

**Quality signals**

- Not enough metadata yet.

**Risks:** No metadata risk notes yet.

## 🧮 Score breakdown

| Dimension | Score |
|---|---:|
| forks | 12/100 |
| stars | 44/100 |
| topics | 63/100 |
| outlook | 62/100 |
| quality | 55/100 |
| recency | 80/100 |
| adoption | 35/100 |
| production | 60/100 |
| usefulness | 58/100 |
| integration | 46/100 |
| sourceTrust | 70/100 |
| categoryMatchCount | 100/100 |

---

<sub>🔭 Discovered 2026-08-30 · [View on GitHub](https://github.com/Calix-L/DanKS) · [← Back to AI/ML](./README.md)</sub>
