...
`attention` `blackwell` `cuda` `deepseek` `diffusion` `glm` `gpt-oss` `inference` `llama` `llm` `minimax` `moe`
...
**Value:** sgl-project/sglang helps add AI capability without starting from a blank model stack.
