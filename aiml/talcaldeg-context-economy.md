# talcaldeg/context-economy

[![Stars](https://img.shields.io/github/stars/talcaldeg/context-economy?style=flat-square&color=yellow)](https://github.com/talcaldeg/context-economy/stargazers) [![Forks](https://img.shields.io/github/forks/talcaldeg/context-economy?style=flat-square&color=blue)](https://github.com/talcaldeg/context-economy/network) [![Language](https://img.shields.io/badge/lang-Python-informational?style=flat-square)](#) [![Score](https://img.shields.io/badge/score-50%2F100-brightgreen?style=flat-square)](#)

> Read-only scripts that measure where a coding agent's tokens actually go, from its own .jsonl transcripts

## 📊 Stats

| | |
|---|---|
| ⭐ **Stars** | — |
| 🍴 **Forks** | — |
| 💻 **Language** | Python |
| 📈 **Score** | 50/100 |
| 🗓️ **Last push** | 2026-09-05 |
| 🔍 **Source** | story-link |

## 🏷️ Topics

`agents` `claude-code` `llm` `observability` `tokens`

## 🎯 Categories

AI/ML · Observability

## 📝 Summary

### English

talcaldeg/context-economy: . Use it for AI/ML, Observability. 

### Русский

talcaldeg/context-economy: open-source проект в категориях AI/ML, Observability. Практическое применение: нужна ручная оценка сценария. Уровень готовности: подходит для прототипа или внутреннего workflow, перед production нужна ручная проверка.

### 中文

talcaldeg/context-economy：。适合用于AI/ML、Observability。

## 🧭 Practical evaluation

**Value:** Manual review required.

**Best use cases**

- Not enough metadata yet.

**Integration notes:** No integration notes yet.

**Production readiness:** Production readiness has not been assessed yet.

**Quality signals**

- Not enough metadata yet.

**Risks:** No metadata risk notes yet.

## 🧮 Score breakdown

| Dimension | Score |
|---|---:|
| forks | 0/100 |
| stars | 0/100 |
| topics | 63/100 |
| outlook | 53/100 |
| quality | 40/100 |
| recency | 80/100 |
| adoption | 0/100 |
| production | 56/100 |
| usefulness | 58/100 |
| integration | 62/100 |
| sourceTrust | 70/100 |
| categoryMatchCount | 200/100 |

---

<sub>🔭 Discovered 2026-09-01 · [View on GitHub](https://github.com/talcaldeg/context-economy) · [← Back to AI/ML](./README.md)</sub>
