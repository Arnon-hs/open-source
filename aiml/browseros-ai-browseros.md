# browseros-ai/BrowserOS

[![Stars](https://img.shields.io/github/stars/browseros-ai/BrowserOS?style=flat-square&color=yellow)](https://github.com/browseros-ai/BrowserOS/stargazers) [![Forks](https://img.shields.io/github/forks/browseros-ai/BrowserOS?style=flat-square&color=blue)](https://github.com/browseros-ai/BrowserOS/network) [![Language](https://img.shields.io/badge/lang-TypeScript-informational?style=flat-square)](#) [![Score](https://img.shields.io/badge/score-100%2F100-brightgreen?style=flat-square)](#)

> 🌐 The open-source Agentic browser; alternative to ChatGPT Atlas, Perplexity Comet, Dia.

## 📊 Stats

| | |
|---|---|
| ⭐ **Stars** | 10.6k |
| 🍴 **Forks** | 1k |
| 💻 **Language** | TypeScript |
| 📈 **Score** | 100/100 |
| 🗓️ **Last push** | 2026-04-23 |
| 🔍 **Source** | github |

## 🏷️ Topics

`agent` `browser` `browseros` `chromium` `hacktoberfest` `linux` `llm` `lmstudio` `macos` `ollama` `windows`

## 🎯 Categories

AI/ML

## 📝 Summary

Here is a 2-3 sentence summary for the BrowserOS open-source project:

BrowserOS is an open-source, agentic browser that offers an alternative to popular AI platforms like ChatGPT, Perplexity, and others. Powered by AI and machine learning, this browser provides a unique interface for users to interact with and control their digital experiences. By harnessing the capabilities of AI, BrowserOS aims to revolutionize the way we navigate and interact with online content.

---

<sub>🔭 Discovered 2026-04-23 · [View on GitHub](https://github.com/browseros-ai/BrowserOS) · [← Back to AI/ML](./README.md)</sub>
