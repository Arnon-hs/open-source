...
`neobrutalism` `neobrutalism-components` `nextjs` `react` `react-component-library` `react-components` `react-typescript` `react-ui-components` `shadcn` `shadcn-components` `shadcn-theme` `shadcn-ui`
...
ekmas/neobrutalism-components：ekmas/neobrutalism-components helps add AI capability without starting from a blank model stack.。适合用于prototype AI features、build RAG or agent workflows。Medium: useful for prototypes or internal workflows, with dependency and maintenance checks before production.
...
<sub>🔭 Discovered 2026-05-10 · [View on GitHub](https://github.com/ekmas/neobrutalism-components) · [← Back to AI/ML](./README.md)</sub>
