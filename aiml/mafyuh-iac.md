...
`actions` `ansible` `devops` `docker` `docker-compose` `gitops` `iac` `k8s` `k8s-at-home` `kubernetes` `kubesearch` `opentofu`
...
Mafyuh/iac：Mafyuh/iac helps add AI capability without starting from a blank model stack.。适合用于prototype AI features、build RAG or agent workflows。Medium: useful for prototypes or internal workflows, with dependency and maintenance checks before production.
...
<sub>🔭 Discovered 2026-05-09 · [View on GitHub](https://github.com/Mafyuh/iac) · [← Back to AI/ML](./README.md)</sub>
