# liyupi/ai-guide

[![Stars](https://img.shields.io/github/stars/liyupi/ai-guide?style=flat-square&color=yellow)](https://github.com/liyupi/ai-guide/stargazers) [![Forks](https://img.shields.io/github/forks/liyupi/ai-guide?style=flat-square&color=blue)](https://github.com/liyupi/ai-guide/network) [![Language](https://img.shields.io/badge/lang-JavaScript-informational?style=flat-square)](#) [![Score](https://img.shields.io/badge/score-100%2F100-brightgreen?style=flat-square)](#)

> 程序员鱼皮的 AI 资源大全 + Vibe Coding 零基础教程，分享 OpenClaw 保姆级教程、大模型玩法（DeepSeek / GPT / Gemini / Claude）、最新 AI 资讯、Prompt 提示词大全、AI 知识百科（Agent Skills / RAG / MCP / A2A）、AI 编程教程（Harness Engineering）、AI 工具用法（Cursor / Claude Code / TRAE / Codex / Copilot）、AI 开发框架教程（Spring AI / LangChain）、AI 产品变现指南，帮你快速掌握 AI 技术，走在时代前沿。本项目为开源文档，已升级为鱼皮 AI 导航网站

## 📊 Stats

| | |
|---|---|
| ⭐ **Stars** | 13k |
| 🍴 **Forks** | 1.5k |
| 💻 **Language** | JavaScript |
| 📈 **Score** | 100/100 |
| 🗓️ **Last push** | — |
| 🔍 **Source** | github |

## 🏷️ Topics

_no topics_

## 🎯 Categories

Orchestration · AI/ML · Frontend

## 📝 Summary

Here's a brief summary of the project in 2-3 sentences:

The ai-guide project is an open-source documentation repository that aggregates AI resources, tutorials, and guides for developers, covering topics from basic AI programming to advanced models and frameworks. It includes comprehensive tutorials on OpenClaw, DeepSeek, GPT, Gemini, and other models, as well as knowledge bases, coding guides, and product monetization strategies. The project aims to help developers quickly gain a grasp on AI technology and stay at the forefront of the field.

---

<sub>🔭 Discovered 2026-04-30 · [View on GitHub](https://github.com/liyupi/ai-guide) · [← Back to Orchestration](./README.md)</sub>
