# hardisgroupcom/sfdx-hardis

[![Stars](https://img.shields.io/github/stars/hardisgroupcom/sfdx-hardis?style=flat-square&color=yellow)](https://github.com/hardisgroupcom/sfdx-hardis/stargazers) [![Forks](https://img.shields.io/github/forks/hardisgroupcom/sfdx-hardis?style=flat-square&color=blue)](https://github.com/hardisgroupcom/sfdx-hardis/network) [![Language](https://img.shields.io/badge/lang-TypeScript-informational?style=flat-square)](#) [![Score](https://img.shields.io/badge/score-54%2F100-brightgreen?style=flat-square)](#)

> French-army-knife Toolbox for Salesforce. Orchestrates base commands and assist users with interactive wizards to make much more than native Salesforce CLI + Allows you to define a complete CI/CD Pipeline and Schedule a daily Metadata backup & monitoring of your orgs + AI-enhanced org Documentation

## 📊 Stats

| | |
|---|---|
| ⭐ **Stars** | 341 |
| 🍴 **Forks** | 94 |
| 💻 **Language** | TypeScript |
| 📈 **Score** | 54/100 |
| 🗓️ **Last push** | — |
| 🔍 **Source** | github |

## 🏷️ Topics

_no topics_

## 🎯 Categories

Crypto · AI/ML · Backend

## 📝 Summary

Here's a brief summary of the open-source project:

The sfdx-hardis project is an open-source, French-army-knife Toolbox for Salesforce, providing a comprehensive set of tools that go beyond the native Salesforce CLI. It simplifies complex tasks through interactive wizards, enables the creation of complete CI/CD pipelines, and offers features like automated daily metadata backups and monitoring. Additionally, it includes AI-enhanced org documentation for streamlined Salesforce management.

---

<sub>🔭 Discovered 2026-04-24 · [View on GitHub](https://github.com/hardisgroupcom/sfdx-hardis) · [← Back to Crypto](./README.md)</sub>
