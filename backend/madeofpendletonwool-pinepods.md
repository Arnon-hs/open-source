# madeofpendletonwool/PinePods

[![Stars](https://img.shields.io/github/stars/madeofpendletonwool/PinePods?style=flat-square&color=yellow)](https://github.com/madeofpendletonwool/PinePods/stargazers) [![Forks](https://img.shields.io/github/forks/madeofpendletonwool/PinePods?style=flat-square&color=blue)](https://github.com/madeofpendletonwool/PinePods/network) [![Language](https://img.shields.io/badge/lang-Rust-informational?style=flat-square)](#) [![Score](https://img.shields.io/badge/score-53%2F100-brightgreen?style=flat-square)](#)

> Pinepods is a complete podcast management system that allows you to play, download, and keep track of podcasts you enjoy. All self hosted and enjoyed on your own server!

## 📊 Stats

| | |
|---|---|
| ⭐ **Stars** | 857 |
| 🍴 **Forks** | 54 |
| 💻 **Language** | Rust |
| 📈 **Score** | 53/100 |
| 🗓️ **Last push** | — |
| 🔍 **Source** | github |

## 🏷️ Topics

_no topics_

## 🎯 Categories

Backend

## 📝 Summary

PinePods is a free, open-source podcast management system designed for self-hosting on your own server. This comprehensive platform enables users to play, download, and track their favorite podcasts in a centralized and customizable environment. With PinePods, you can take control of your podcast library and enjoy seamless management of your favorite shows.

---

<sub>🔭 Discovered 2026-04-30 · [View on GitHub](https://github.com/madeofpendletonwool/PinePods) · [← Back to Backend](./README.md)</sub>
