# aywengo/kafka-schema-reg-mcp

[![Stars](https://img.shields.io/github/stars/aywengo/kafka-schema-reg-mcp?style=flat-square&color=yellow)](https://github.com/aywengo/kafka-schema-reg-mcp/stargazers) [![Forks](https://img.shields.io/github/forks/aywengo/kafka-schema-reg-mcp?style=flat-square&color=blue)](https://github.com/aywengo/kafka-schema-reg-mcp/network) [![Language](https://img.shields.io/badge/lang-Python-informational?style=flat-square)](#) [![Score](https://img.shields.io/badge/score-70%2F100-brightgreen?style=flat-square)](#)

> A comprehensive Message Control Protocol (MCP) server for Kafka Schema Registry.

## 📊 Stats

| | |
|---|---|
| ⭐ **Stars** | 32 |
| 🍴 **Forks** | 9 |
| 💻 **Language** | Python |
| 📈 **Score** | 70/100 |
| 🗓️ **Last push** | 2026-08-22 |
| 🔍 **Source** | github |

## 🏷️ Topics

`apache-kafka` `claude-desktop` `cursor` `docker` `enterprise` `kafka` `mcp` `mcp-server` `schema-registry` `vscode`

## 🎯 Categories

MCP · Backend · DevOps/Infra

## 📝 Summary

### English

aywengo/kafka-schema-reg-mcp: aywengo/kafka-schema-reg-mcp helps connect AI assistants to real tools and data through a standard protocol.. Use it for connect AI agents to tools; ship Model Context Protocol servers. High for an OSS candidate: recent activity, adoption, and ecosystem signals are strong enough for a serious pilot.

### Русский

aywengo/kafka-schema-reg-mcp: open-source проект в категориях MCP, Backend, DevOps/Infra. Практическое применение: connect AI agents to tools; ship Model Context Protocol servers. Уровень готовности: готов для серьезного pilot с проверкой license, security и maintainer activity.

### 中文

aywengo/kafka-schema-reg-mcp：aywengo/kafka-schema-reg-mcp helps connect AI assistants to real tools and data through a standard protocol.。适合用于connect AI agents to tools、ship Model Context Protocol servers。High for an OSS candidate: recent activity, adoption, and ecosystem signals are strong enough for a serious pilot.

## 🧭 Practical evaluation

**Value:** aywengo/kafka-schema-reg-mcp helps connect AI assistants to real tools and data through a standard protocol.

**Best use cases**

- connect AI agents to tools
- ship Model Context Protocol servers
- standardize integrations

**Integration notes:** Looks straightforward to evaluate: it exposes implementation signals such as API/SDK/CLI, language metadata, or focused topics.

**Production readiness:** High for an OSS candidate: recent activity, adoption, and ecosystem signals are strong enough for a serious pilot.

**Quality signals**

- 32 GitHub stars
- 9 forks
- updated 2026-08-22
- primary language: Python
- 10 topics

**Risks:** No major metadata risk found, but license, security posture, and active maintainers still need final review.

## 🧮 Score breakdown

| Dimension | Score |
|---|---:|
| forks | 25/100 |
| stars | 32/100 |
| topics | 100/100 |
| outlook | 74/100 |
| quality | 69/100 |
| recency | 100/100 |
| adoption | 30/100 |
| production | 79/100 |
| usefulness | 58/100 |
| integration | 94/100 |
| sourceTrust | 100/100 |

---

<sub>🔭 Discovered 2026-08-22 · [View on GitHub](https://github.com/aywengo/kafka-schema-reg-mcp) · [← Back to Mcp](./README.md)</sub>
